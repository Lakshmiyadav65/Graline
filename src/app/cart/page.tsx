...
export default function CartPage() {
  ...
  if (!mounted) {
    return (
      <PageShell>
        <div className="py-24 text-center text-muted text-[14px]">Loading your cart…</div>
      </PageShell>
    );
  }
      ...
      <PageShell>
        <section className="py-16 max-w-xl">
          ...
          <h1 className="font-serif text-[clamp(32px,5vw,48px)] font-normal mt-4 mb-3">
            Your cart is <em className="text-terra font-medium">empty</em>.
          </h1>
          <p className="text-ink-soft text-[16px] mb-8">
            Browse this season&apos;s harvest and add rice from a farmer you can name.
          </p>
          <Link
            href="/browse"
  ...
  return (
    <PageShell>
      <section className="py-12">
        <Eyebrow>Your cart · {cartCount(items)} item{cartCount(items) === 1 ? "" : "s"}</Eyebrow>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-8">
          ...
          <div className="bg-cream border border-line rounded-card-lg p-6 h-fit">
            <h3 className="font-serif text-[22px] font-medium mb-4">Summary</h3>
            <Row label="Subtotal" value={formatRupees(subtotal)} />
            {saving > 0 && (
              <Row label="Direct-trade saving vs retail" value={`−${formatRupees(saving)}`} tone="paddy" />
            )}
            <p className="text-[12px] text-muted mt-3 mb-4">
              Delivery fees and your Saturday delivery date ({formatDate(getNextSaturday())}) are
              set at checkout once you pick a fulfillment option.
            </p>
            <button
              type="button"
