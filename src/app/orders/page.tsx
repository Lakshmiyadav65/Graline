...
function itemSummary(order: Order): string {
  ...
  const label = first ? `${VARIETY_LABEL[first.variety as RiceVariety] ?? first.variety} ${first.pack_kg}kg` : "";
    ...
    <PageShell>
      <section className="py-12">
        ...
        <h1 className="font-serif text-[clamp(32px,5vw,48px)] font-normal mt-3.5 mb-8">
          Order <em className="text-terra font-medium">history</em>.
        </h1>

        {orders === null ? (
          <div className="text-muted text-[14px] py-12">Loading orders…</div>
        ) : orders.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-line rounded-card-lg">
            <p className="font-serif text-[22px] mb-2">No orders yet.</p>
            <Link href="/browse" className="inline-flex mt-3 px-5 py-3 bg-ink text-paper rounded-full text-[13px] font-medium hover:bg-paddy transition-all">Browse rice →</Link>
          </div>
        ) : (
            ...
            {orders.map((o) => (
              ...
              >
                ...
                <span className="justify-self-end">
                  <Pill tone={orderStatusTone(o.status)}>{ORDER_STATUS_LABEL[o.status]}</Pill>
                </span>
              </Link>
            ))}
          </div>
